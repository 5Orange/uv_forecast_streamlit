...
**Chức năng.**
Lấy dữ liệu thời tiết và khí quyển theo thời gian thực từ các API bên ngoài, trích xuất 23 đặc trưng đầu vào cho mô hình, và tạo ra các dự báo UV theo giờ cho cả 7 địa điểm trong khoảng thời gian 7 ngày.
...
- `_compute_all_features()` — xây dựng tất cả 23 đặc trưng từ `config.FINAL_23_FEATURES`.
...
|---|---|---|---|---|
| **Kính Trong nhà làm suy giảm** | `Effective_UV = UV × 0.05` | `effective_uv = uv * 0.05` | Kính kiến trúc tiêu chuẩn chặn >95% tia UVB gây đỏ da. | Tuchinda et al. (2006). "Photoprotection by window glass, automobile glass, and sunglasses". *J Am Acad Dermatol*. |
...
**Ý nghĩa Khoa học.**
Thay vì cộng điểm thưởng tuyến tính, các địa điểm trong nhà theo logic sẽ chịu mức giảm 95% trong Effective UV (UV hiệu dụng), điều này về mặt toán học sẽ mở rộng `Safe_Minutes` đến một khoảng thời gian cực dài, tự nhiên đẩy `SafeRatio` lên mức 1.0. Đối với các khu vực bóng râm ngoài trời, một mô hình truyền qua vật lý tương tự sẽ tính toán mức độ phơi nhiễm UV kết hợp giữa bầu trời mở và bầu trời được lọc qua tán cây. Giới hạn căng thẳng do nhiệt được áp dụng như một hệ số nhân suy giảm sinh lý cứng để phạt các địa điểm lộ thiên ngoài trời dưới điều kiện nhiệt độ quá cao.
...
`_compute_solar()` tính toán các góc mặt trời qua `pvlib`. `_compute_all_features()` xây dựng 23 đặc trưng đầu vào. `_predict()` chạy mô hình đã huấn luyện trên các dòng thời gian ban ngày, áp dụng tỷ lệ mặt trời và rút ra phân loại UV qua các ngưỡng chuẩn của WHO.
...
**Điểm mạnh:**
- Các công thức vật lý (MED, kính, bóng râm, khoảng cách, căng thẳng nhiệt) đều có nền tảng học thuật quốc tế vững chắc (WHO, CIE, ISO 7243).
...
- **Xếp hạng Sinh lý Thuần túy (Pure Physiological Ranking):** Các bài kiểm thử biên (edge cases) nhấn mạnh rằng hệ thống chỉ xếp hạng theo mức độ an toàn UV về mặt sinh lý. Điều này dẫn đến sự không khớp về mặt ngữ nghĩa: nếu người dùng muốn "đi biển" nhưng một "công viên" lại có bóng râm an toàn hơn, hệ thống sẽ đề xuất công viên và phớt lờ mong muốn "đi biển" của người dùng. Điều này giải thích tại sao một số bài test biên lại thất bại. (Tỷ lệ pass chung: 72.7%).
